...
function validateLinks(l, i) {
  ...
  if ( o.sync.ignoreESPTags === '1' && o.sync.espMergeTags ) {
    ...
    o.sync.espMergeTags.forEach(function (esp, index) {
      ...
      if ( l._urlInDOM ) {
        ...
        let mergeTagMatches = l._urlInDOM.match(re);
        ...
        if ( mergeTagMatches ) {
          mergeTagMatches.forEach(function (tag, index) {
            ...
            l._urlInDOMMergeTagSafe = l._urlInDOMMergeTagSafe.replace(tag, 'espmergetagplaceholder');
  ...
  // console.log(l);
  // console.log("l._urlInDOM", l._urlInDOM);
  // console.log("l._urlInDOMMergeTagSafe", l._urlInDOMMergeTagSafe);
  ...
  var ell;
  if ( l._urlInDOM ) {
    if ( l._urlInDOM.length > 70 ) {
      ell = l._urlInDOM.substring(0,70) + "...";
    } else {
      ell = l._urlInDOM;
    }
  }
  ...
  linkMarker.dataset.href = l._urlInDOM;
  ...
  linkErrorLogURL.innerHTML = "<korra-div class='link-number'>#" + (i + 1) + "</korra-div><korra-div class='link-url'>" + l._urlInDOM + "</korra-div>";

  //
  // var linkErrorLogURLTextNode = document.createTextNode(l._urlInDOM);
  ...
  singleLinkInfoArray.url = l._urlInDOM; //link url
  singleLinkInfoArray.text = l.textContent.trim(); //link text
  singleLinkInfoArray.imgsLinked = l.querySelectorAll('img'); // images linked
  singleLinkInfoArray.querystring = decipherQuerystring(l._urlInDOM, l); //querystring
  ...
  // Is this the first time this link appears in the DOM?
	if ( xhrLoopArray.includes(l._urlInDOM) ) {
  ...
  xhrLoopArray.push(l._urlInDOM); // Add it to the array.
  singleLinkInfoArray.instanceOrder = countInsideArray(xhrLoopArray, l._urlInDOM); // Apply an order #. (eg. the 4th instance of this exact link in the DOM)
